...
from matchbox.core.models import Bullet, Facet, ItemType, Project, Skill, SummaryVariant, Tag
...
def add_bullet(body: BulletBody, conn: ConnDep) -> dict[str, Any]:
    b = lib.add_bullet(
        conn, experience_id=body.experience_id, text=body.text.strip(), has_metric=body.has_metric
    )
...
def add_skill(body: SkillBody, conn: ConnDep) -> dict[str, Any]:
    ...
    try:
        s = lib.add_skill(
            conn, name=body.name.strip(), category=body.category, proficiency=body.proficiency
        )
