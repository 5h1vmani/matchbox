...
const { useState: useRevState, useMemo: useRevMemo, useEffect: useRevFx } = React;

function fitRank(level) { return { strong: 0, good: 1, stretch: 2 }[level] ?? 3; }

/* full review card */
function ReviewCard({ role, onDecide, onOpenJD, aside }) {
  const eligIneligible = role.eligibility.status === "ineligible";
  const closed = role.freshness === "closed";
  return (
    <div className={window.dcx("rcard", aside && "aside")}>
        ...
        <div className="rcard__head">
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <div className="rcard__co">
              ...
              <span>{window.fullLoc(role)}</span>
              {role.salary && <React.Fragment><span className="dotsep" /><span className="sal">{role.salary}</span></React.Fragment>}
            </div>
...
function Review({ roles, onDecide, onOpenJD, onGoBrowse, flash }) {
  const [asideOpen, setAsideOpen] = useRevState(false);

  // active queue: eligible/unclear + open, undecided. Closing-soon first, then fit.
  const queue = useRevMemo(() =>
    roles.filter((r) => !r.decision && r.eligibility.status !== "ineligible" && r.freshness !== "closed")
      .sort((a, b) => {
        ...
        if (a.freshness === "closing" && b.freshness === "closing") return a.closingInDays - b.closingInDays;
        return fitRank(a.fit.level) - fitRank(b.fit.level) || a.postedDaysAgo - b.postedDaysAgo;
      }), [roles]);

  // set aside: ineligible or closed, undecided
  const aside = useRevMemo(() =>
    roles.filter((r) => !r.decision && (r.eligibility.status === "ineligible" || r.freshness === "closed")),
  [roles]);

  // tally of decisions made this session
  const tally = useRevMemo(() => {
    const t = { tracked: 0, tailoring: 0, dismissed: 0 };
  ...
  return (
    <div className="review">
      ...
      {aside.length > 0 && (
        <div className={window.dcx("aside-group", asideOpen && "open")}>
