/* Matchbox — Application detail drawer. JTBD #3: log, advance, note, prep —
   without leaving the page. Slides in from the right over the tracker/today. */
const { useState: useDetState, useRef: useDetRef, useEffect: useDetFx } = React;

function whenText(daysAgo) {
...
function Detail({ app, actions, flash, onClose, focusNote }) {
  const [note, setNote] = useDetState("");
  const noteRef = useDetRef(null);
  useDetFx(() => { if (focusNote && noteRef.current) noteRef.current.focus(); }, [focusNote]);
    ...
    <div className="scrim" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="drawer" onMouseDown={(e) => e.stopPropagation()}>
        <div className="drawer__top">
          ...
          <span className="sp" />
          <window.StarBtn app={app} actions={actions} size={16} />
          ...
          <window.QuickButton app={app} actions={actions} flash={flash} onOpen={() => {}} />
        </div>
        ...
        <div className="drawer__body">
          <div className="dhdr">
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <div className="tags">
                <window.Badge tone={closed ? "neutral" : "cool"} dot>{window.stageLabel(app.stage)}</window.Badge>
                <window.Badge tone="neutral"><Icon name="map-pin" size={11} /> {app.location}</window.Badge>
                <window.Badge tone="neutral"><Icon name="banknote" size={11} /> {app.salary}</window.Badge>
              </div>
            </div>
          </div>

          <window.StageStepper stage={app.stage} />

          {/* Next action card */}
          {a && a.kind !== "wait" ? (
            <div className="dnext">
              <span className="di"><Icon name={window.eventIcon(a.kind === "followup" ? "followup" : a.kind === "interview" ? "screen" : a.kind)} size={19} /></span>
              <div style={{ minWidth: 0 }}>
                <div className="l">{a.label}{a.time ? " · " + a.time : ""}</div>
                <div className="s">{a.due !== null ? window.dueInfo(a.due).text : "No date set"}{app.hasDraft && (a.kind === "followup" || a.kind === "thanks") ? " · draft ready" : ""}</div>
              </div>
              <div className="cta">
          ...
          <div className="dsec">
            ...
            <div className="qgrid">
              {nextStage && (
                <button className="qbtn" onClick={() => { actions.advanceStage(app.id); flash("Moved to " + window.stageLabel(nextStage).toLowerCase()); }}>
                  <Icon name="arrow-right" size={17} />
                  <span>Advance stage<span className="s">to {window.stageLabel(nextStage).toLowerCase()}</span></span>
                </button>
              )}
          ...
          <div className="dsec">
            ...
            <div className="notebox">
              <textarea ref={noteRef} placeholder="Jot a note — what to ask, who you met, how it felt…"
            ...
            <div className="timeline">
              {app.events.map((ev, idx) => (
                <div className="tl" key={idx}>
                  <span className={window.cx("tl__dot", tlTone(ev.kind))}><Icon name={window.eventIcon(ev.kind)} size={14} /></span>
