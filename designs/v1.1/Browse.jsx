...
const { useState: useBrState, useMemo: useBrMemo } = React;

function ChipRead({ role }) {
  const fm = window.FIT_META[role.fit.level] || window.FIT_META.good;
  return (
...
function RoleTile({ role, selected, onToggleSel, onOpen, onDecide }) {
  const dimmed = role.eligibility.status === "ineligible" || role.freshness === "closed";
  return (
    <div className={window.dcx("rtile", selected && "sel", dimmed && "dimmed")} onClick={() => onOpen(role)}>
      <button className={window.dcx("rtile__sel", selected && "on")} title="Select"
      ...
      <div className="rtile__head">
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          <div className="rtile__co">{role.company} · {window.fullLoc(role)}</div>
        </div>
      </div>
...
function Browse({ roles, sel, onToggleSel, onClearSel, onOpen, onDecide, onBatch, flash }) {
  const [eligibleOnly, setEligibleOnly] = useBrState(true);
  const [fresh, setFresh] = useBrState("all");
  const [fit, setFit] = useBrState("all");
  const [remoteOnly, setRemoteOnly] = useBrState(false);

  const list = useBrMemo(() => {
    return roles.filter((r) => {
      ...
      return ({ strong: 0, good: 1, stretch: 2 }[a.fit.level]) - ({ strong: 0, good: 1, stretch: 2 }[b.fit.level]);
    });
    ...
    <div>
      ...
      <div className="filters">
        <div className="fgroup">
          {FIT_FILTERS.map((f) => (
            <button key={f.id} className={window.dcx("fchip", fit === f.id && "active")} onClick={() => setFit(f.id)}>
        ...
        <div className="fgroup">
          {FRESH_FILTERS.map((f) => (
            <button key={f.id} className={window.dcx("fchip", fresh === f.id && "active")} onClick={() => setFresh(f.id)}>{f.label}</button>
          ))}
        </div>
        <span className="fsep" />
        <button className={window.dcx("fchip", "toggle", eligibleOnly && "active")} onClick={() => setEligibleOnly((v) => !v)}>
          <Icon name={eligibleOnly ? "check" : "circle"} size={13} /> Eligible only
        </button>
        <button className={window.dcx("fchip", "toggle", remoteOnly && "active")} onClick={() => setRemoteOnly((v) => !v)}>
