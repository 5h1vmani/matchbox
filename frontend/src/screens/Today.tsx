/* Matchbox — Today / Focus home. JTBD #1: "tell me what to do now."
   ...
   Ported verbatim from designs/v1/Today.jsx. */
import { useMemo, useState } from "react";
import type { Application, Direction, Flash, OpenDetail, TrackerActions } from "../types";
import { stageLabel } from "../data/stages";
import { actionPhrase, cx, dueInfo, dueVal, RANK } from "../lib/derive";
import { Icon } from "../ui/icon";
import { ActionLine, Badge, Due, MonoLogo } from "../ui/atoms";
import { QuickButton } from "../ui/parts";
...
function TaskRow({ app, actions, flash, onOpen, done, onToggle }: TaskRowProps) {
  ...
  return (
    <div className="task">
      <button className={cx("tick", isDone && "done")} title="Mark done" onClick={() => onToggle(app)}>
        {isDone && <Icon name="check" size={12} />}
      </button>
      <div className="body" onClick={() => onOpen(app)}>
        <div className={cx("lead", isDone && "struck")}>
          <ActionLine app={app} />
        </div>
...
function UpNext({ app, onOpen }: { app: Application; onOpen: OpenDetail }) {
...
export function Today({ apps, actions, flash, onOpen, dir, loading }: TodayProps) {
  ...
  const toggleDone = (app: Application) => {
    if (done[app.id]) {
      setDone((d) => { const n = { ...d }; delete n[app.id]; return n; });
      return;
    }
  ...
  const upcoming = useMemo(() =>
    apps
      .filter((a) => a.nextAction && a.nextAction.kind === "interview" && a.nextAction.due !== null && a.nextAction.due >= 0 && a.nextAction.due <= 7)
      .sort((x, y) => dueVal(x) - dueVal(y))
      .slice(0, 4),
  [apps]);

  const drafts = useMemo(() => apps.filter((a) => a.hasDraft && a.stage !== "rejected").slice(0, 4), [apps]);
  const cold = useMemo(() => apps.filter((a) => a.stale), [apps]);
  const offers = useMemo(() => apps.filter((a) => a.stage === "offer"), [apps]);
    ...
    <div className="today-wrap">
      <div className="greet">
        ...
        <div className="glance">
          ...
          {offers.length > 0 && (
            <div className="g"><div className="v" style={{ color: "var(--success)" }}>{offers.length}</div><div className="k">offer{offers.length > 1 ? "s" : ""}</div></div>
          )}
        </div>
      </div>
          ...
          {top ? (
            <div className="hero__row">
              ...
              <div style={{ minWidth: 0 }}>
                <div className="hero__lead"><ActionLine app={top} /></div>
                <div className="hero__sub">
                  {actionPhrase(top).sub}
                  {top.nextAction && top.nextAction.due !== null && top.nextAction.due <= 0 ? " · due now" : ""}
                </div>
              </div>
          ...
          <>
            <div className="tasks">
              {(expanded ? listSource : listSource.slice(0, CAP)).map((app) => (
                <TaskRow key={app.id} app={app} actions={actions} flash={flash} onOpen={onOpen} done={done[app.id]} onToggle={toggleDone} />
              ))}
            </div>
      ...
      {cold.length > 0 && (
        <div className={cx("cold", coldOpen && "open")}>
              ...
              {cold.map((app) => (
                <div className="coldrow" key={app.id}>
                  ...
                  <div className="info" onClick={() => onOpen(app)}>
                    ...
                    <div className="s">No update in {app.updatedDaysAgo} days · {stageLabel(app.stage)}</div>
                  </div>
                  <div className="acts">
