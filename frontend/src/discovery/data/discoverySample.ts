/* Matchbox — Discovery sample data. Roles the system surfaces for triage.
   Ported verbatim from designs/v1.1/discovery-data.js (the dev/offline fixture
   and the screenshot-parity fixture). The student: US-based product designer /
   design engineer, ~5 yrs, React + design systems, no EU work authorization,
   open to remote. That profile drives the fit + eligibility reads below.
   Hand-authored for honest, specific reasons. */
import type { Role, WatchedCompany } from "../types";
...
const m = (co: string): { bg: string; fg: string } => {
  const x = mono[co] || ["#ede8e8", "#574747"];
...
let _id = 0;
function r(
  company: string, title: string, location: string, remote: boolean, salary: string | null,
  source: string, posted: number, fitLevel: Role["fit"]["level"], fitReason: string,
  elig: Role["eligibility"]["status"], eligReason: string, fresh: Role["freshness"],
  closing: number | null, covered: number | null, total: number | null,
): Role {
  _id++;
  return {
    ...
    coverage: total ? { covered: covered as number, total } : null,
    mono: m(company),
...
export const SAMPLE_ROLES = ROLES;
export const SAMPLE_WATCH = WATCH;
